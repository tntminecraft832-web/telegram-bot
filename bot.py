...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("🎵 موسیقی\n🎬 لینک\n🎙️ ویس\nℹ️ راهنما", reply_markup=MAIN_KB)

async def search_yt(query):
    q = urllib.parse.quote(query)
    url = "https://www.googleapis.com/youtube/v3/search?part=snippet&q=" + q + "&maxResults=1&type=video&key=" + API_KEY
...
async def handle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "🎵 موسیقی از یوتیوب":
        ...
        await update.message.reply_text("اسم آهنگ:")
        return
    if text == "🎬 لینک ویدیو":
        context.user_data["mode"] = "link"
        await update.message.reply_text("لینک:")
        return
    ...
    try:
        if mode == "music":
            ...
            await dl_audio("https://youtu.be/" + vid, "s.mp3")
            ...
            with open("t.txt", "w", encoding="utf-8") as f:
                f.write(text)
            subprocess.run(["edge-tts", "--voice", "fa-IR-FaridNeural", "-f", "t.txt", "--write-media", "voice.mp3"], check=True)
            await update.message.reply_voice(voice=open("voice.mp3", "rb"))
            kb = InlineKeyboardMarkup([[InlineKeyboardButton("📥 دانلودر", callback_data="dl_tts")]])
            await update.message.reply_text("ویدیو هم بسازم؟", reply_markup=kb)
            os.remove("t.txt")
        await msg.delete()
    except Exception as e:
        await msg.edit_text("❌ خطا: " + str(e))
